...
- Персонал: 3 сотрудника (`LAB_ADMIN`) и 4 преподавателя (`TEACHER`).
...
- `Room`: 15 записей.
- `Discipline`: 31 запись.
- `LabWork`: 36 записей (детализированные ЛР для 3 дисциплин + базовые ЛР остальных дисциплин).
- `LabSession`: минимум 72 открытых слота при `--weeks 2`.
- `User`:
  - 3 сотрудника лаборатории (`LAB_ADMIN`);
  - 4 преподавателя (`TEACHER`);
  - 90 студентов (`STUDENT`).
- `UserProfile`:
  - для студентов: `group_name`, `student_id`, `dekanat_id` (по CSV);
  - для сотрудников/преподавателей: `training_center`.
...
- `seed_demo` — основной канал для согласованного набора пилотных данных нефтегазового факультета.
- `import_dekanat_csv` — дозагрузка/обновление реальных людей и дисциплин из CSV (`students`, `teachers`, `staff`, `disciplines`).
...
- Ограничения "какая группа видит какую дисциплину" пока задаются организационно (через публикацию/данные), отдельной матрицы доступа по группам в модели нет.
